...
//		public byte[]	pImage;	// The pointer to the captured image
	...
	public struct CAMERAINFO 
	{
		...
		public double	dExposureExMin;	// Min value of Shutter Width(Return -1 if disabled)
		public double	dExposureExMax;	// Max value of Shutter Width(Return -1 if disabled)
	}
...
*/
	...
	public class CArtCam
	{
		...
		[UnmanagedFunctionPointer(CallingConvention.Cdecl)] delegate int SETINTERNALCORRECTION			(int hACam, int  Enable);
		[UnmanagedFunctionPointer(CallingConvention.Cdecl)] delegate int GETINTERNALCORRECTION			(int hACam, out int outError);
		...
		[UnmanagedFunctionPointer(CallingConvention.Cdecl)] delegate double	GETTEMPERATUREEX			(int hACam, out int outError);
		[UnmanagedFunctionPointer(CallingConvention.Cdecl)] delegate int	SETEXPOSURETIMEEX			(int hACam, double Value);
		[UnmanagedFunctionPointer(CallingConvention.Cdecl)] delegate double	GETEXPOSURETIMEEX			(int hACam, out int outError);
		...
		private	SETINTERNALCORRECTION			m_SetInternalCorrection;
		private	GETINTERNALCORRECTION			m_GetInternalCorrection;
		...
		private	GETTEMPERATUREEX				m_GetTemperatureEx;
		private	SETEXPOSURETIMEEX				m_SetExposureTimeEx;
		private	GETEXPOSURETIMEEX				m_GetExposureTimeEx;
		...
		public bool LoadLibrary(string szDllName)
		{
			...
			if(1280 <= Version){
				...
				if (ARTCAM_CAMERATYPE.ARTCAM_CAMERATYPE_130MI_MOUT == (ARTCAM_CAMERATYPE)DllType || ARTCAM_CAMERATYPE.ARTCAM_CAMERATYPE_150P3_MOUT == (ARTCAM_CAMERATYPE)DllType ||
					ARTCAM_CAMERATYPE.ARTCAM_CAMERATYPE_1000MI_HD2 == (ARTCAM_CAMERATYPE)DllType)
				{
			...
			if(1300 <= Version){
				if (ARTCAM_CAMERATYPE.ARTCAM_CAMERATYPE_CNV != (ARTCAM_CAMERATYPE)DllType)
				{
					...
					if(null == m_GetRealExposureTime || null == m_SetRealExposureTime){
						FreeLibrary();
						return false;
					}
				}

				switch ((ARTCAM_CAMERATYPE)DllType)
				{
					...
					case ARTCAM_CAMERATYPE.ARTCAM_CAMERATYPE_032TNIR_USB3_T2:
					case ARTCAM_CAMERATYPE.ARTCAM_CAMERATYPE_009TNIR_USB3_T2:
					case ARTCAM_CAMERATYPE.ARTCAM_CAMERATYPE_131TNIR_USB3_T2:
					case ARTCAM_CAMERATYPE.ARTCAM_CAMERATYPE_130SWIR_USB3_T2:
						m_SaveMaskFile		=	(SAVEMASKFILE)	GetDelegate("ArtCam_SaveMaskFile",		typeof(SAVEMASKFILE));
						m_LoadMaskFile		=	(LOADMASKFILE)	GetDelegate("ArtCam_LoadMaskFile",		typeof(LOADMASKFILE));
						m_UpdateMaskData	=	(UPDATEMASKDATA)GetDelegate("ArtCam_UpdateMaskData",	typeof(UPDATEMASKDATA));
						...
						m_SetInternalCorrection = (SETINTERNALCORRECTION)GetDelegate("ArtCam_SetInternalCorrection", typeof(SETINTERNALCORRECTION));
						m_GetInternalCorrection = (GETINTERNALCORRECTION)GetDelegate("ArtCam_GetInternalCorrection", typeof(GETINTERNALCORRECTION));
						if (null == m_SetInternalCorrection || null == m_GetInternalCorrection)
						{
							FreeLibrary();
							return false;
						}
			...
			if(1312 <= Version){
                m_GetTemperatureEx = (GETTEMPERATUREEX)GetDelegate("ArtCam_GetTemperatureEx", typeof(GETTEMPERATUREEX));
                m_SetExposureTimeEx = (SETEXPOSURETIMEEX)GetDelegate("ArtCam_SetExposureTimeEx", typeof(SETEXPOSURETIMEEX));
                m_GetExposureTimeEx = (GETEXPOSURETIMEEX)GetDelegate("ArtCam_GetExposureTimeEx", typeof(GETEXPOSURETIMEEX));
			}
			return true;
		}
		...
		public int SetInternalCorrection(int Enable){		
			if(null!= m_SetInternalCorrection){	
				return m_SetInternalCorrection(m_hACam, Enable);	
			} 
			return 0;	
		}

		public int GetInternalCorrection()
		{		
			if(null!= m_GetInternalCorrection){	
				return m_GetInternalCorrection(m_hACam, out m_Error);	
			} 
			return 0;	
		}
		...
		public double GetTemperatureEx()
		{		
			if(null!= m_GetTemperatureEx){	
				return m_GetTemperatureEx(m_hACam, out m_Error);	
			} 
			return 0.0;	
		}

		public int SetExposureTimeEx(double Value)
		{		
			if(null!= m_SetExposureTimeEx){	
				return m_SetExposureTimeEx(m_hACam, Value);	
			} 
			return 0;	
		}

		public double GetExposureTimeEx()
		{		
			if(null!= m_GetExposureTimeEx){	
				return m_GetExposureTimeEx(m_hACam, out m_Error);	
			} 
			return -1.0;	
		}
